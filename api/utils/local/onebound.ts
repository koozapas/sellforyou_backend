...
export const getTranslateData = (taobaoData: IOBItem, isTranslated?: boolean) => {
	...
	const firstPropertyInfo =
		taobaoData.skus.sku.length === 0
			? undefined
			: taobaoData.skus.sku[0]?.properties_name?.match(/[-\d]+?:[-\d]+?:(.+?):([^;]+);?/g);
		...
		taobaoProducts
			...
			.map(async (v) => {
				...
				if (!product) {
					...
					var description = (translateData?.description ?? taobaoData.desc).replace(
						/(?<!<p ?>)(<img [^>]*?>)(?!<p>)/g,
						'<p>$1</p>',
					);
					...
					product = await prisma.product.create({
						data: {
							...
							imageThumbnailData: JSON.stringify(
								taobaoData.item_imgs.map((v) => 'https:' + v.url.replace(/^https?:/, '')),
							),
							productCode: '',
							...
							searchTags:
								taobaoData.desc_short !== '' ? taobaoData.desc_short.replace(regex, '') : searchTags.replace(regex, ''),
					...
					if (taobaoData.shop_id === 'vvic' || taobaoData.shop_id === 'express') {
							...
							var new_imgs = await raceAll(
								taobaoData.item_imgs.map(async (v, i) => {
									try {
										...
										let image_base64 =
											'data:' +
											(isWebp ? 'image/jpg' : image_resp.headers['content-type']) +
											';base64,' +
											image_raw.toString('base64');
						...
						if (
							translateData &&
							taobaoData.desc_img.length > 0 &&
							(taobaoData.shop_id === 'vvic' || taobaoData.shop_id === 'express')
						) {
							...
							if (desc_imgs.length > 0)
								description = (translateData?.description ?? taobaoData.desc).replace(
									/(?<!<p ?>)(<img [^>]*?>)(?!<p>)/g,
									'<p>$1</p>',
								);
						}
					...
					if (res) {
						...
						const productOptionValues = await Promise.all(
							Object.entries(taobaoData.props_list).map(async ([key, value], i) => {
								...
								const name =
									translateData?.optionValue.find((v2) => v2.taobaoPid === a[1] && v2.taobaoVid === a[3])?.name ?? b[2]; //매칭시켜버림 옵션명과 옵션내용
