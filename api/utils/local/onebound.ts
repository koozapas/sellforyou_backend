...
export const saveTaobaoItemToUser = async <T extends IFeeInfo>(
  ...
  taobaoProducts: ((TaobaoProduct & { itemData: IOBItem; translateDataObject: ITranslateData | null }) | null)[],
  userId: number | null,
  userInfo: T,
  ...
  return await Promise.all(
    taobaoProducts
      .filter((v): v is TaobaoProduct & { itemData: IOBItem; translateDataObject: ITranslateData | null } => v !== null)
      .map(async (v) => {
            ...
            if (taobaoData.item_imgs.length > 0) {
              var new_imgs = await raceAll(
                taobaoData.item_imgs.map(async (v, i) => {
                  try {
                    let image_resp: any = await axios.get(v.url, { responseType: "arraybuffer" });
            ...
            if (translateData && taobaoData.desc_img.length > 0) {
              var desc_imgs = await raceAll(
                taobaoData.desc_img.map(async (v, i) => {
                  try {
                    let image_resp: any = await axios.get(v, { responseType: "arraybuffer" });
          ...
          if (res) {
            ...
            await Promise.all(
              taobaoData.skus.sku.map(async (sku) => {
                ...
                const match = sku.properties.match(
                  /^([-\d]+):([-\d]+);?([-\d]+)?:?([-\d]+)?;?([-\d]+)?:?([-\d]+)?;?([-\d]+)?:?([-\d]+)?;?([-\d]+)?:?([-\d]+)?/
                )!; //옵션5개로변경
