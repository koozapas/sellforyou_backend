...
export async function getOptionHeaderHtmlByProductId(
  prisma: PrismaClient,
  id: number,
  twoWays: string,
  indexType: number,
  shopName: any,
  useDetailInformation: any
): Promise<string> {
...
export function getOptionHeaderHtml(
  optionValues: (ProductOptionValue & { productOptionName: ProductOptionName })[],
  twoWays: string,
  indexType: number,
  shopName: any,
  useDetailInformation: any
        ...
        for (var i in options) {
          ...
          output += `
                        <div style="border: 1px solid lightgray; padding: 4%;">
                            ...
                            ${
                              options[i].image
                                ? `<p style="text-align: center; width: 600px; margin: auto;"><img src="${getValidUploadImageUrl(
                                    imageUrl,
                                    shopName
                                  )}" alt="" width= "100%" /></p>`
                                : "<p>&nbsp;</p>"
          ...
          output += `
                        <td style="border: 1px solid lightgray; width: 50%; padding: 4%;">
                            <div>
                                ...
                                ${
                                  options[i].image
                                    ? `<p style="text-align: center;"><img src="${getValidUploadImageUrl(
                                        imageUrl,
                                        shopName
                                      )}" alt="" width="100%" /></p>`
                                    : "<p>&nbsp;</p>"
