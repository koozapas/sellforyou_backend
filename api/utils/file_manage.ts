...
export const uploadToS3AvoidDuplicate = async (
	pfile: FileUpload,
...
export const uploadToS3AvoidDuplicateByBuffer = async (
	...
	filename = fileNameExcludeExtension
		? fileNameExcludeExtension + filename.replace(regexPattern.fileNameAndExtension, '.$2')
		: filename;
...
export const uploadToS3WithEditor = async (
	content: string,
	pathArray: (string | number)[],
	fileNameExcludeExtension: string | null,
) => {
	/** img태그에 잡다한 속성들 제거 */
	...
	if (result) {
		const urlArray = await Promise.all(
			result.map(async (v, i) => {
				const a = v
					.replace(/<img(?: align=".*")? src="?data:(image\/.*?);base64,(.*?)"? ?\/?>/g, '$1*$*~$2')
					.split('*$*~');
...
export const uploadToS3WithEditor2 = async (
	content: string,
	pathArray: (string | number)[],
	fileNameExcludeExtension: string | null,
) => {
