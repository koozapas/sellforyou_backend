...
const isNeedToTranslate = <T extends { name: string }>(object: T) =>
	!/^[\x00-\x7F가-힣ㄱ-ㅎㅏ-ㅣ]+$/g.test(object.name);

const translateProduct = async (
	ctx: Context,
	type: NexusGenAllTypes['TranslateTargetEnumType'],
	id: number,
): Promise<string> => {
	let ids: number[] = [];
	...
	if (['PRODUCT_ALL', 'PRODUCT_NAME'].includes(type)) {
		const product = await ctx.prisma.product.findUnique({
			where: { id: id },
			include: { productOptionName: { select: { id: true } } },
		});
			...
			await Promise.all(
				ids.map(async (id) => {
					const productOptionName = await ctx.prisma.productOptionName.findUnique({
						where: { id },
						include: { productOptionValue: { select: { id: true } } },
					});
					if (!productOptionName) return throwError(errors.noSuchData, ctx);
					if (!productOptionName.isNameTranslated) {
						const name = isNeedToTranslate(productOptionName)
							? await translateTextByPapagoAPI(productOptionName.name)
						...
						await ctx.prisma.productOptionName.update({
							...
							data: { isNameTranslated: true, name },
						});
					}
					if (['PRODUCT_ALL', 'PRODUCT_OPTION_ALL'].includes(type))
						return productOptionName.productOptionValue.map((v) => v.id);
					return null;
				}),
			)
		)
		...
		await Promise.all(
			ids.map(async (id) => {
				...
				if (!productOptionValue.isNameTranslated) {
					...
					const name = isNeedToTranslate(productOptionValue)
						? await translateTextByPapagoAPI(productOptionValue.name)
		...
		t.field('translateProductsTextByUser', {
			...
			resolve: async (src, args, ctx, info) => {
				try {
					...
					else if (args.ids.length === failedCount)
						return `${failedCount}개의 상품 모두 이미 번역된 ${args.type === 'PRODUCT_ALL' ? '상품' : '항목'}입니다.`;
