...
const truncateOptionNameInternal = (siteCode: string, text: string) => {
    // replace(/[\{\}\[\]\/?.,;:|\)*~`!^\-_+<>@\#$%&\\\=\(\'\"]/gi, "")
    switch(siteCode){
        case 'A077' : return text.replace(/[\\*?"<>]/g, "").slice(0, 25); //스마트스토어
        case 'B378' : return text.slice(0, 30);  //쿠팡
        case 'A112' : return text.replace(/[',"%&<>#†\\∏‡]/g, "").slice(0, 25);//11번가
        case 'A113' : return text.replace(/[',"%&<>#†\\∏‡]/g, "").slice(0, 25);//11번가
        case 'A006' : return byteSlice(text, 40);//G marget
        case 'A001' : return text.replace(/[\[\]\^,'"\\@&;]/g, "").slice(0, 25);//auction 
        case 'A027' : return text.replace(/[^[\]a-zA-Zㄱ-ㅎㅏ-ㅣ가-힣0-9\- ]/g, "").slice(0, 25);//interpark
        case 'B719' : return text.slice(0, 25);//wemake
        case 'A524' : return text.slice(0, 25);//lotteon
        case 'A525' : return text.slice(0, 25);//lotteon
        case 'B956' : return text.slice(0, 25);//tmon developing
        default : return text.slice(0, 25);
    }
                        ...
                        DataDataSet: {
                            ...
                            data_opt: productStore.map((v : any , i : number) => {//여기부터 
                                ...
                                if (vSiteCode === 'A077') { //스마트스토어 할인금액 설정
                                    ...
                                    if (v.product.productOption.some((v2 :any) => v2.price < minRange || v2.price > maxRange)) { //판매가 범위 밖이면
                                ...
                                const a = v.product.productOption.map((v2 :any ,index2 : number) => {
                                    const image = v2.productOption1.image ?? v2.productOption2?.image ?? v2.productOption3?.image ?? null;
    
                                    let opt_price = Math.round((v2.price - price) / calculateWonType) * calculateWonType;
    
                                    if (vSiteCode === 'A077') {
                                        opt_price = Math.floor(opt_price / (100 -  (v.product.naverFee !== null ? v.product.naverFee :userInfo?.naverFee)) * ( 100 / calculateWonType)) * calculateWonType;
                                    } else if (vSiteCode === 'B378') {
                                        opt_price = Math.floor(opt_price / (100 - (v.product.coupangFee !== null ? v.product.coupangFee :userInfo?.coupangFee))* ( 100 / calculateWonType)) * calculateWonType;
                                    } else if (vSiteCode === 'A112') {
                                        opt_price = Math.floor(opt_price / (100 - (v.product.streetFee !== null ? v.product.streetFee :userInfo?.streetFee))* ( 100 / calculateWonType)) * calculateWonType;
                                    } else if (vSiteCode === 'A001') {
                                        opt_price = Math.floor(opt_price / (100 - (v.product.auctionFee !== null ? v.product.auctionFee :userInfo?.auctionFee))* ( 100 / calculateWonType)) * calculateWonType;
                                    } else if (vSiteCode === 'A006') {
                                        opt_price = Math.floor(opt_price / (100 - (v.product.gmarketFee !== null ? v.product.gmarketFee :userInfo?.gmarketFee))* ( 100 / calculateWonType)) * calculateWonType;
                                    } else if (vSiteCode === 'A027') {
                                        opt_price = Math.floor(opt_price / (100 - (v.product.interparkFee !== null ? v.product.interparkFee :userInfo?.interparkFee))* ( 100 / calculateWonType)) * calculateWonType;
                                    } else if (vSiteCode === 'A113') {
                                        opt_price = Math.floor(opt_price / (100 - (v.product.streetNormalFee !== null ? v.product.streetNormalFee :userInfo?.streetNormalFee))* ( 100 / calculateWonType)) * calculateWonType;
                                    } else if (vSiteCode === 'B719') {
                                        opt_price = Math.floor(opt_price / (100 - (v.product.wemakepriceFee !== null ? v.product.wemakepriceFee :userInfo?.wemakepriceFee))* ( 100 / calculateWonType)) * calculateWonType;
                                    } else if (vSiteCode === 'A524') {
                                        opt_price = Math.floor(opt_price / (100 - (v.product.lotteonFee !== null ? v.product.lotteonFee :userInfo?.lotteonFee))* ( 100 / calculateWonType)) * calculateWonType;
                                    } else if (vSiteCode === 'A525') {
                                        opt_price = Math.floor(opt_price / (100 - (v.product.lotteonNormalFee !== null ? v.product.lotteonNormalFee :userInfo?.lotteonNormalFee))* ( 100 / calculateWonType)) * calculateWonType;
                                    } else if (vSiteCode === 'B956') {
                                        opt_price = Math.floor(opt_price / (100 - (v.product.tmonFee !== null ? v.product.tmonFee :userInfo?.tmonFee))* ( 100 / calculateWonType)) * calculateWonType;
                                    }
                            ...
                            data: await Promise.all(productStore.map(async v => {
                                ...
                                //카테고리 및 고시정보
                                const siil_data = v.product.siilCode ? JSON.stringify(predefinedSiilData.find(v2 => v2.infoCode === v.product.siilCode)!) : JSON.stringify(predefinedSiilData.find(v2 => v2.infoCode === "35")!);
                                //카테고리 고시정보
                                let cate1 = '';
                                let cate2 = '';
                                let cate3 = '';
                                ...
                                if (vSiteCode === 'A077') {
                                    price = Math.ceil(price / (100 - (v.product.naverFee !== null ? v.product.naverFee : userInfo?.naverFee))* ( 100 / calculateWonType)) * calculateWonType;
                                    discount = Math.floor(discount / (100 - (v.product.naverFee !== null ? v.product.naverFee : userInfo?.naverFee))* ( 100 / calculateWonType)) * calculateWonType;
                                } else if (vSiteCode === 'B378') {
                                     price = Math.ceil(price / (100 - (v.product.coupangFee !== null ? v.product.coupangFee : userInfo?.coupangFee))* ( 100 / calculateWonType)) * calculateWonType;
                                     discount = Math.ceil(discount / (100 - (v.product.coupangFee !== null ? v.product.coupangFee : userInfo?.coupangFee))* ( 100 / calculateWonType)) * calculateWonType;
                                } else if (vSiteCode === 'A112') {
                                     price = Math.ceil(price / (100 - (v.product.streetFee !== null ? v.product.streetFee : userInfo?.streetFee))* ( 100 / calculateWonType)) * calculateWonType;
                                     discount = Math.ceil(discount / (100 - (v.product.streetFee !== null ? v.product.streetFee : userInfo?.streetFee))* ( 100 / calculateWonType)) * calculateWonType;
                                 } else if (vSiteCode === 'A027') {
                                     price = Math.ceil(price / (100 - (v.product.interparkFee !== null ? v.product.interparkFee : userInfo?.interparkFee))* ( 100 / calculateWonType)) * calculateWonType;
                                     discount = Math.ceil(discount / (100 - (v.product.interparkFee !== null ? v.product.interparkFee : userInfo?.interparkFee))* ( 100 / calculateWonType)) * calculateWonType;
                                 } else if (vSiteCode === 'A001') {
                                     price = Math.ceil(price / (100 - (v.product.auctionFee !== null ? v.product.auctionFee : userInfo?.auctionFee))* ( 100 / calculateWonType)) * calculateWonType;
                                     discount = Math.ceil(discount / (100 - (v.product.auctionFee !== null ? v.product.auctionFee : userInfo?.auctionFee))* ( 100 / calculateWonType)) * calculateWonType;
                                 } else if (vSiteCode === 'A006') {
                                    price = Math.ceil(price / (100 - (v.product.gmarketFee !== null ? v.product.gmarketFee : userInfo?.gmarketFee))* ( 100 / calculateWonType)) * calculateWonType;
                                    discount = Math.ceil(discount / (100 - (v.product.gmarketFee !== null ? v.product.gmarketFee : userInfo?.gmarketFee))* ( 100 / calculateWonType)) * calculateWonType;
                                } else if (vSiteCode === 'A113') {
                                    price = Math.ceil(price / (100 - (v.product.streetNormalFee !== null ? v.product.streetNormalFee : userInfo?.streetNormalFee))* ( 100 / calculateWonType)) * calculateWonType;
                                    discount = Math.ceil(discount / (100 - (v.product.streetNormalFee !== null ? v.product.streetNormalFee : userInfo?.streetNormalFee))* ( 100 / calculateWonType)) * calculateWonType;
                                } else if (vSiteCode === 'B719') {
                                    price = Math.ceil(price / (100 - (v.product.wemakepriceFee !== null ? v.product.wemakepriceFee : userInfo?.wemakepriceFee))* ( 100 / calculateWonType)) * calculateWonType;
                                    discount = Math.ceil(discount / (100 - (v.product.wemakepriceFee !== null ? v.product.wemakepriceFee : userInfo?.wemakepriceFee))* ( 100 / calculateWonType)) * calculateWonType;
                                } else if (vSiteCode === 'A524') {
                                    price = Math.ceil(price / (100 - (v.product.lotteonFee !== null ? v.product.lotteonFee : userInfo?.lotteonFee))* ( 100 / calculateWonType)) * calculateWonType;
                                    discount = Math.ceil(discount / (100 - (v.product.lotteonFee !== null ? v.product.lotteonFee : userInfo?.lotteonFee))* ( 100 / calculateWonType)) * calculateWonType;
                                } else if (vSiteCode === 'A525') {
                                    price = Math.ceil(price / (100 - (v.product.lotteonNormalFee !== null ? v.product.lotteonNormalFee : userInfo?.lotteonNormalFee))* ( 100 / calculateWonType)) * calculateWonType;
                                    discount = Math.ceil(discount / (100 - (v.product.lotteonNormalFee !== null ? v.product.lotteonNormalFee : userInfo?.lotteonNormalFee))* ( 100 / calculateWonType)) * calculateWonType;
                                } else if (vSiteCode === 'B956') {
                                    price = Math.ceil(price / (100 - (v.product.tmonFee  !== null ? v.product.tmonFee  : userInfo?.tmonFee)) * ( 100 / calculateWonType)) * calculateWonType;
                                    discount = Math.ceil(discount / (100 - (v.product.tmonFee  !== null ? v.product.tmonFee  : userInfo?.tmonFee)) * ( 100 / calculateWonType)) * calculateWonType;
                                }
                                ...
                                return {
                                    ...
                                    content: userInfo.optionAlignTop === "N" ? 
                                                userInfo.useDetailInformation === "N"?  `<div style="text-align: center;">${userInfo.fixImageTop ? "<img src=\"" + getValidUploadImageUrl(userInfo.fixImageTop,v.product.taobaoProduct.shopName) + "?" + Date.now() + "\" alt=\"\" />" : ""}${userInfo.fixImageSubTop ? "<img src=\"" + getValidUploadImageUrl(userInfo.fixImageSubTop,v.product.taobaoProduct.shopName) + "?" + Date.now() + "\" alt=\"\" />" : ""}<p>&nbsp;</p><p>&nbsp;</p>${/^product\/\d+\/description.html$/.test(v.product.description) ? (await getFromS3(v.product.description)).Body!.toString("utf8") : v.product.description}${await getOptionHeaderHtmlByProductId(ctx.prisma, v.product.id, userInfo?.optionTwoWays, userInfo?.optionIndexType,v.product.taobaoProduct.shopName,userInfo.useDetailInformation)}${userInfo.fixImageBottom ? "<img src=\"" + getValidUploadImageUrl(userInfo.fixImageBottom,v.product.taobaoProduct.shopName)  + "?" + Date.now() +  "\" alt=\"\" />" : ""}${userInfo.fixImageSubBottom ? "<img src=\"" + getValidUploadImageUrl(userInfo.fixImageSubBottom,v.product.taobaoProduct.shopName) + "?" + Date.now() + "\" alt=\"\" />" : ""}</div>`
                                        ...
                                        :  userInfo.useDetailInformation === "N"? `<div style="text-align: center;">${userInfo.fixImageTop ? "<img src=\"" + getValidUploadImageUrl(userInfo.fixImageTop,v.product.taobaoProduct.shopName) + "?" + Date.now() + "\" alt=\"\" />" : ""}${userInfo.fixImageSubTop ? "<img src=\"" + getValidUploadImageUrl(userInfo.fixImageSubTop,v.product.taobaoProduct.shopName) + "?" + Date.now() + "\" alt=\"\" />" : ""}${await getOptionHeaderHtmlByProductId(ctx.prisma, v.product.id, userInfo?.optionTwoWays, userInfo?.optionIndexType,v.product.taobaoProduct.shopName,userInfo.useDetailInformation)}<p>&nbsp;</p><p>&nbsp;</p>${/^product\/\d+\/description.html$/.test(v.product.description) ? (await getFromS3(v.product.description)).Body!.toString("utf8") : v.product.description}${userInfo.fixImageBottom ? "<img src=\"" + getValidUploadImageUrl(userInfo.fixImageBottom,v.product.taobaoProduct.shopName)  + "?" + Date.now() +  "\" alt=\"\" />" : ""}${userInfo.fixImageSubBottom ? "<img src=\"" + getValidUploadImageUrl(userInfo.fixImageSubBottom,v.product.taobaoProduct.shopName) + "?" + Date.now() + "\" alt=\"\" />" : ""}</div>`
                                        :`<div style="text-align: center;">${userInfo.fixImageTop ? "<img src=\"" + getValidUploadImageUrl(userInfo.fixImageTop,v.product.taobaoProduct.shopName) + "?" + Date.now() + "\" alt=\"\" />" : ""}${userInfo.fixImageSubTop ? "<img src=\"" + getValidUploadImageUrl(userInfo.fixImageSubTop,v.product.taobaoProduct.shopName) + "?" + Date.now() + "\" alt=\"\" />" : ""}${await getOptionHeaderHtmlByProductId(ctx.prisma, v.product.id, userInfo?.optionTwoWays, userInfo?.optionIndexType,v.product.taobaoProduct.shopName,userInfo.useDetailInformation)}<p>&nbsp;</p><p>&nbsp;</p><p><div style="color: #000000; font-size: 24px; font-weight: bold; font-family: none;">상품 설명입니다.</div></p><p>&nbsp;</p><p>&nbsp;</p>${/^product\/\d+\/description.html$/.test(v.product.description) ? (await getFromS3(v.product.description)).Body!.toString("utf8") : v.product.description}${userInfo.fixImageBottom ? "<img src=\"" + getValidUploadImageUrl(userInfo.fixImageBottom,v.product.taobaoProduct.shopName)  + "?" + Date.now() +  "\" alt=\"\" />" : ""}${userInfo.fixImageSubBottom ? "<img src=\"" + getValidUploadImageUrl(userInfo.fixImageSubBottom,v.product.taobaoProduct.shopName) + "?" + Date.now() + "\" alt=\"\" />" : ""}</div>`,
                                    
                                    content1 : userInfo.optionAlignTop === "N" ?  
                                                userInfo.useDetailInformation === "N"?  `<div style="text-align: center;"><p>&nbsp;</p><p>&nbsp;</p>${/^product\/\d+\/description.html$/.test(v.product.description) ? (await getFromS3(v.product.description)).Body!.toString("utf8") : v.product.description}${await getOptionHeaderHtmlByProductId(ctx.prisma, v.product.id, userInfo?.optionTwoWays, userInfo?.optionIndexType,v.product.taobaoProduct.shopName,userInfo.useDetailInformation)}</div>`
                                        :`<div style="text-align: center;"><p>&nbsp;</p><p>&nbsp;</p><p><div style="color: #000000; font-size: 24px; font-weight: bold; font-family: none;">상품 설명입니다.</div></p><p>&nbsp;</p><p>&nbsp;</p>${/^product\/\d+\/description.html$/.test(v.product.description) ? (await getFromS3(v.product.description)).Body!.toString("utf8") : v.product.description}${await getOptionHeaderHtmlByProductId(ctx.prisma, v.product.id, userInfo?.optionTwoWays, userInfo?.optionIndexType,v.product.taobaoProduct.shopName,userInfo.useDetailInformation)}</div>`
                                        :  userInfo.useDetailInformation === "N"?   `<div style="text-align: center;">${await getOptionHeaderHtmlByProductId(ctx.prisma, v.product.id, userInfo?.optionTwoWays, userInfo?.optionIndexType,v.product.taobaoProduct.shopName,userInfo.useDetailInformation)}<p>&nbsp;</p><p>&nbsp;</p>${/^product\/\d+\/description.html$/.test(v.product.description) ? (await getFromS3(v.product.description)).Body!.toString("utf8") : v.product.description}</div>`
                                        :`<div style="text-align: center;">${await getOptionHeaderHtmlByProductId(ctx.prisma, v.product.id, userInfo?.optionTwoWays, userInfo?.optionIndexType,v.product.taobaoProduct.shopName,userInfo.useDetailInformation)}<p>&nbsp;</p><p>&nbsp;</p><p><div style="color: #000000; font-size: 24px; font-weight: bold; font-family: none;">상품 설명입니다.</div></p><p>&nbsp;</p><p>&nbsp;</p>${/^product\/\d+\/description.html$/.test(v.product.description) ? (await getFromS3(v.product.description)).Body!.toString("utf8") : v.product.description}</div>`,
