			...
			resolve: async (src, args, ctx, info) => {
				try {
					...
					const attachmentFile = args.attachment
						? (await uploadToS3(args.attachment, ['notice', notice.id])).url
						: null;
			...
			resolve: async (src, args, ctx, info) => {
				try {
					...
					const content = args.content
						? await uploadToS3WithEditor(args.content, ['notice', notice.id], null)
						: undefined;
		...
		t.field('createUserQuestionByUser', {
			...
			resolve: async (src, args, ctx, info) => {
				try {
					...
					if (args.title.length === 0) return throwError(errors.etc('제목을 입력하세요.'), ctx);
					if (args.title.length > 200) return throwError(errors.etc('제목은 200자 이하로 입력해주세요.'), ctx);
					if (args.attachment && args.attachment.length > 5)
						return throwError(errors.etc('첨부파일은 5개 이하만 업로드 가능합니다.'), ctx);
		...
		t.field('updateUserQuestionByAdmin', {
			...
			resolve: async (src, args, ctx, info) => {
				try {
					...
					const content = args.answer
						? await uploadToS3WithEditor(args.answer, ['userQuestion', userQuestion.id], null)
						: undefined;
