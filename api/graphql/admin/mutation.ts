	...
	definition(t) {
		t.field('signUpAdminByAdmin', {
			...
			resolve: async (src, args, ctx, info) => {
				try {
					if (await ctx.prisma.admin.findUnique({ where: { loginId: args.id } }))
						return throwError(errors.etc('해당 이메일이 이미 존재합니다.'), ctx);
					const admin = await ctx.prisma.admin.create({
						data: { loginId: args.id, password: hashSync(args.password), state: 'ACTIVE' },
					});
					return true;
				} catch (e) {
				...
				try {
					switch (args.store) {
						...
						case 'lotteon':
							...
							break;
						default:
							return throwError(errors.etc('스토어명이 존재하지 않습니다.'), ctx);
					}
		...
		t.field('changeMyPasswordByAdmin', {
			...
			resolve: async (src, args, ctx, info) => {
				try {
					...
					if (!compareSync(args.currentPassword, admin.password))
