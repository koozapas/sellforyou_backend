...
const initProductImageByUser = async (
	...
	try {
		...
		let imageThumbnailData = JSON.stringify(
			taobaoData.item_imgs.map((v: any) => 'http:' + v.url.replace(/^https?:/, '')),
		);
		let desc_html = ``;
...
const updateProductCategory2 = async (
	...
	if (args.categoryA077) {
		...
		if (args.categoryB378) {
			result = await ctx.prisma.categoryInfoB378.findUnique({
				where: { code: args.categoryB378 },
				include: { sillInfoB378: true },
			});
			...
			result = await ctx.prisma.categoryInfoA112.findUnique({
				where: { code: args.categoryA112 },
				include: { sillInfoA112: true },
			});
			...
			result = await ctx.prisma.categoryInfoA027.findUnique({
				where: { code: args.categoryA027 },
				include: { sillInfoA027: true },
			});
			...
			result = await ctx.prisma.categoryInfoA001.findUnique({
				where: { code: args.categoryA001 },
				include: { sillInfoA001: true },
			});
			...
			result = await ctx.prisma.categoryInfoA006.findUnique({
				where: { code: args.categoryA006 },
				include: { sillInfoA006: true },
			});
			...
			result = await ctx.prisma.categoryInfoB719.findUnique({
				where: { code: args.categoryB719 },
				include: { sillInfoB719: true },
			});
			...
			result = await ctx.prisma.categoryInfoA113.findUnique({
				where: { code: args.categoryA113 },
				include: { sillInfoA113: true },
			});
			...
			result = await ctx.prisma.categoryInfoA524.findUnique({
				where: { code: args.categoryA524 },
				include: { sillInfoA524: true },
			});
			...
			result = await ctx.prisma.categoryInfoA525.findUnique({
				where: { code: args.categoryA525 },
				include: { sillInfoA525: true },
			});
			...
			result = await ctx.prisma.categoryInfoB956.findUnique({
				where: { code: args.categoryB956 },
				include: { sillInfoB956: true },
			});
...
const updateProductResolver = async (
	...
	try {
		...
		if (args.siilCode) {
			if (siilInfo.findIndex((v) => v.infoCode === args.siilCode) === -1)
				return throwError(errors.etc('잘못된 상품고시정보입니다.'), ctx);
		}

		const siil: SiilEncodedSavedData | null =
			args.siilCode && args.siilData
				? { c: args.siilCode, d: args.siilData!.map((v) => ({ c: v.code, v: v.value })) }
				: null;
					...
					args.options.map(async (v) => {
						if (v.id === w.id) {
							await ctx.prisma.productOption.update({
								where: { id: v.id },
								data: {
									price: Math.round(v.price / calculateWonType) * calculateWonType,
									isActive: true,
									stock: v.stock,
								},
							});
		...
		const result = await ctx.prisma.product.update({
			...
			data: {
				...
				localShippingFee: args.localShippingFee
					? Math.round(args.localShippingFee / calculateWonType) * calculateWonType
					: undefined,
...
const updateDescription = async (
	src: {},
	args: ArgsValue<'Mutation', 'updateDescription'>,
	ctx: Context,
	info: GraphQLResolveInfo,
) => {
	try {
		...
		await Promise.all(
			args.data.map(async (v: any) => {
				...
				const description = v.description
					? await uploadToS3WithEditor2(v.description, ['product', product.id], 'description')
					: undefined;
				if (!description) return throwError(errors.etc('description 업데이트 과정에 문제가 생겼습니다.'), ctx);
				await ctx.prisma.product.update({
					where: { id: product.id },
...
const deleteUserResolver = async (
	src: {},
	args: ArgsValue<'Mutation', 'deleteUserByAdmin'>,
	ctx: Context,
	info: GraphQLResolveInfo,
) => {
	try {
...
const updateProductFee = async (
	src: {},
	args: ArgsValue<'Mutation', 'updateProductFee'>,
	ctx: Context,
	info: GraphQLResolveInfo,
) => {
	try {
...
export const mutation_product = extendType({
	...
	definition(t) {
					...
					for (let v of args.optionValues) {
						...
						if (v.newImageBase64) {
							...
							if (res) {
								...
								image = await uploadToS3AvoidDuplicateByBuffer(buffer, `option_${v.id}_.${ext}`, mimetype, [
									'product',
									product.id,
								]);
							} else {
						...
						for (let v of args.thumbnails) {
							...
							if (v.uploadImageBase64) {
								...
								if (res) {
									...
									image = await uploadToS3AvoidDuplicateByBuffer(buffer, `thumbnail.${ext}`, mimetype, [
										'product',
										product.id,
									]);
								}
					...
					for (let [index, value] of args.optionValues.entries()) {
						...
						if (value.newImageBase64) {
							...
							if (res) {
								...
								image = await uploadToS3AvoidDuplicateByBuffer(buffer, `option_${value.id}_.${ext}`, mimetype, [
									'product',
									product.id,
								]);
						...
						result.map(async (v: any) => {
							...
							if (!productStore) {
								...
								return {
									...
									reason: v.msg,
									state: v.state,
									storeUrl: v.slave_reg_code,
								};
							} else {
								const productStoreState =
									v.state === 1 ? { connect: { id: ProductStoreStateEnum.ON_SELL } } : undefined;
								const etcVendorItemId = v.site_code === 'B378' ? v.slave_reg_code_sub : undefined;
								const updatedResult = await ctx.prisma.productStore.update({
									where: { id: productStore.id },
									data: {
										storeProductId:
											v.state !== 2 ? (v.slave_reg_code !== '' ? v.slave_reg_code : undefined) : undefined,
										productStoreState: productStoreState,
										productStoreLog:
											v.state === 2
												? {
		...
		t.field('updateManyKeywardList', {
			...
			resolve: async (src, args, ctx, info) => {
				try {
					let product: any = await ctx.prisma.product.updateMany({
						where: { id: { in: args.productIds } },
			...
			resolve: async (src, args, ctx, info) => {
				try {
					if (args.productOptionValueId) {
						...
						if (args.newImage) {
							image = await uploadToS3AvoidDuplicate(args.newImage, [
								'product',
						...
						await ctx.prisma.productOptionValue.update({
							...
							data: args.name
								? { isActive: args.isActive, name: args.name.trim(), image }
								: { isActive: args.isActive, image },
						});
		...
		t.field('disableUserOption', {
			...
			resolve: async (src, args, ctx, info) => {
				try {
					...
					const productOptionValue = await ctx.prisma.productOptionValue.findMany({
						where: { productOptionNameId: productOptionName.id },
					});
							...
							if (v.optionValue1Id && !v.optionValue2Id && !v.optionValue3Id) {
								...
								await ctx.prisma.productOption.update({
									...
									data: {
										...
										optionValue2Id:
											productOptionName.order === 1 || productOptionName.order === 2
												? v.optionValue3Id ?? null
												: undefined,
										optionValue3Id: null,
									},
								});
		...
		t.field('changeProductCategoryCode', {
			type: nonNull('Int'),
			args: {
				shopCode: nonNull('String'),
				data: nonNull(list(nonNull(arg({ type: 'ChangeProductCategoryCodeInput' })))),
			},
			resolve: async (src: {}, args, ctx, info) => {
				let cursor = 0;
				let totalCount = 0;
					...
					switch (args.shopCode) {
						case 'A113':
							for (let codePair of codePairs) {
								...
								console.log(
									`${updatedProduct.count}개의 상품이 ${codePair.oldCode}에서 ${codePair.newCode} 로 변경되었습니다.\n`,
								);
								cursor += 1;
