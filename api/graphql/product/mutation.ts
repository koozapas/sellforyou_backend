    ...
    let result : any ;
     await ctx.prisma.product.update(
        ...
        if(args.categoryA077){
            ...
            sillInfoB378 : [...new Set([{ code : result.categoryInfoB378.sillInfoB378.code , name : result.categoryInfoB378.sillInfoB378.name , data : result.categoryInfoB378.sillInfoB378.data }, { code : '기타 재화' , name : '기타 재화', data : '[{"code": "품명 및 모델명", "name": "품명 및 모델명", "type": "input"}, {"code": "인증/허가 사항", "name": "인증/허가 사항", "type": "input"}, {"code": "제조국(원산지)", "name": "제조국(원산지)", "type": "input"}, {"code": "제조자(수입자)", "name": "제조자(수입자)", "type": "input"}, {"code": "소비자상담 관련 전화번호", "name": "소비자상담 관련 전화번호", "type": "input"}]'}])],
            sillInfoA112 : [...new Set([{ code : result.categoryInfoA112.sillInfoA112.code , name : result.categoryInfoA112.sillInfoA112.name , data : result.categoryInfoA112.sillInfoA112.data }, { code : '891045' , name : '기타 재화', data : '[{"code": "23759100", "name": "제조국 또는 원산지", "type": "input"}, {"code": "23756033", "name": "법에 의한 인증·허가 등을 받았음을 확인할 수 있는 경우 그에 대한 사항", "type": "input"}, {"code": "11905", "name": "제조자/수입자", "type": "input"}, {"code": "23760413", "name": "A/S 책임자와 전화번호 또는 소비자상담 관련 전화번호", "type": "input"}, {"code": "11800", "name": "품명 및 모델명", "type": "input"}]'}])],
            sillInfoA027 : [...new Set([{ code : result.categoryInfoA027.sillInfoA027.code , name : result.categoryInfoA027.sillInfoA027.name , data : result.categoryInfoA027.sillInfoA027.data }, { code : '38' , name : '기타', data : '[{"code": "3801", "name": "품명 및 모델명", "type": "input"}, {"code": "3802", "name": "소유권 이전 조건", "type": "input"}, {"code": "3803", "name": "상품의 고장·분실·훼손 시 소비자 책임", "type": "input"}, {"code": "3804", "name": "중도 해약 시 환불 기준", "type": "input"}, {"code": "3805", "name": "소비자상담 관련 전화번호", "type": "input"}, {"code": "3806", "name": "비고", "type": "input"}]'}])],
            sillInfoA001 : [...new Set([{ code : result.categoryInfoA001.sillInfoA001.code , name : result.categoryInfoA001.sillInfoA001.name , data : result.categoryInfoA001.sillInfoA001.data }, { code : '35' , name : '기타', data : '[{"code": "35-1", "name": "품명 및 모델명", "type": "input"}, {"code": "35-2", "name": "허가 관련", "type": "input"}, {"code": "35-3", "name": "제조국 또는 원산지", "type": "input"}, {"code": "35-4", "name": "제조자/수입자", "type": "input"}, {"code": "35-5", "name": "관련 연락처", "type": "input"}, {"code": "35-6", "name": "주문후 예상 배송기간", "type": "input"}]'}])],
            sillInfoA006 : [...new Set([{ code : result.categoryInfoA006.sillInfoA006.code , name : result.categoryInfoA006.sillInfoA006.name , data : result.categoryInfoA006.sillInfoA006.data }, { code : '35' , name : '기타', data : '[{"code": "35-1", "name": "품명 및 모델명", "type": "input"}, {"code": "35-2", "name": "허가 관련", "type": "input"}, {"code": "35-3", "name": "제조국 또는 원산지", "type": "input"}, {"code": "35-4", "name": "제조자/수입자", "type": "input"}, {"code": "35-5", "name": "관련 연락처", "type": "input"}, {"code": "35-6", "name": "주문후 예상 배송기간", "type": "input"}]'}])],
            sillInfoB719 : [...new Set([{ code : result.categoryInfoB719.sillInfoB719.code , name : result.categoryInfoB719.sillInfoB719.name , data : result.categoryInfoB719.sillInfoB719.data }, { code : '38' , name : '기타 재화', data : '[{"code": "195", "name": "품명 및 모델명", "type": "input"}, {"code": "197", "name": "법에 의한 인증, 허가 등을 받았음을 확인할 수 있는 경우 그에 대한 사항", "type": "input"}, {"code": "198", "name": "제조국 또는 원산지", "type": "input"}, {"code": "201", "name": "제조자, 수입품의 경우 수입자를 함께 표기", "type": "input"}, {"code": "203", "name": "A/S 책임자와 전화번호 또는 소비자상담 관련 전화번호", "type": "input"}]'}])],
            sillInfoA113 : [...new Set([{ code : result.categoryInfoA113.sillInfoA113.code , name : result.categoryInfoA113.sillInfoA113.name , data : result.categoryInfoA113.sillInfoA113.data }, { code : '891045' , name : '기타 재화', data : '[{"code": "23759100", "name": "제조국 또는 원산지", "type": "input"}, {"code": "23756033", "name": "법에 의한 인증·허가 등을 받았음을 확인할 수 있는 경우 그에 대한 사항", "type": "input"}, {"code": "11905", "name": "제조자/수입자", "type": "input"}, {"code": "23760413", "name": "A/S 책임자와 전화번호 또는 소비자상담 관련 전화번호", "type": "input"}, {"code": "11800", "name": "품명 및 모델명", "type": "input"}]'}])],
            sillInfoA524 : [...new Set([{ code : result.categoryInfoA524.sillInfoA524.code , name : result.categoryInfoA524.sillInfoA524.name , data : result.categoryInfoA524.sillInfoA524.data }, { code : '38' , name : '기타(재화)', data : '[{"code": "0210", "name": "1. 품명 및 모델명", "type": "input"}, {"code": "1400", "name": "2. 법에 의한 인증ㆍ허가 등을 받았음을 확인할 수 있는 경우 그에 대한 사항", "type": "input"}, {"code": "1420", "name": "3. 제조국 또는 원산지", "type": "input"}, {"code": "0070", "name": "4. 제조자, 수입품의 경우 수입자를 함께 표기", "type": "input"}, {"code": "1440", "name": "5. A/S 책임자와 전화번호 또는 소비자 상담 관련 전화번호", "type": "input"}]'}])],
            sillInfoA525 : [...new Set([{ code : result.categoryInfoA525.sillInfoA525.code , name : result.categoryInfoA525.sillInfoA525.name , data : result.categoryInfoA525.sillInfoA525.data }, { code : '38' , name : '기타(재화)', data : '[{"code": "0210", "name": "1. 품명 및 모델명", "type": "input"}, {"code": "1400", "name": "2. 법에 의한 인증ㆍ허가 등을 받았음을 확인할 수 있는 경우 그에 대한 사항", "type": "input"}, {"code": "1420", "name": "3. 제조국 또는 원산지", "type": "input"}, {"code": "0070", "name": "4. 제조자, 수입품의 경우 수입자를 함께 표기", "type": "input"}, {"code": "1440", "name": "5. A/S 책임자와 전화번호 또는 소비자 상담 관련 전화번호", "type": "input"}]'}])],
            sillInfoB956 : [...new Set([{ code : result.categoryInfoB956.sillInfoB956.code , name : result.categoryInfoB956.sillInfoB956.name , data : result.categoryInfoB956.sillInfoB956.data }, { code : '31' , name : '기타 재화', data : '[{"code": "1166", "name": "1. 품명 및 모델명", "type": "input"}, {"code": "1170", "name": "2. 법에 의한 인증·허가 등을 받았음을 확인할 수 있는 경우 그에 대한 사항", "type": "input"}, {"code": "1174", "name": "3. 제조국 또는 원산지", "type": "input"}, {"code": "1178", "name": "4. 제조자, 수입품의 경우 수입자를 함께 표기", "type": "input"}, {"code": "1182", "name": "5. A/S 책임자와 전화번호 또는 소비자상담 관련 전화번호", "type": "input"}]'}])]
            });
        }else {
