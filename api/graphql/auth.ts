        ...
        t.field("requestPhoneVerificationByEveryone",{
            ...
            resolve: async ( src,args,ctx,info) => {
                try { 
                    ...
                    const verification = await ctx.prisma.phoneVerification.findFirst({ where : {
                        tel,
                        used : 0
        ...
        t.field("verifyPhoneByEveryone", {
            ...
            resolve: async (src, args, ctx, info) => {
                try {
                    ...
                    if(differenceInMinutes(new Date(), verification.createdAt) > 1) return throwError(errors.etc("인증시간이 경과되었습니다."),ctx);
