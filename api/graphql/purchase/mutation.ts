				...
				try {
					...
					if (plan.externalFeatureVariableId) {
						if (existingInfo.additionalInfo.find((v) => v.type === plan.externalFeatureVariableId))
							return throwError(errors.etc('이미 해당 상품을 이용중입니다.'), ctx);
					}
					await ctx.prisma.purchaseLog.create({
						data: {
							type:
								plan.planLevel !== null
									? 'PLAN'
									: (plan.externalFeatureVariableId as NexusGenAllTypes['PurchaseLogType']), //TODO: 나중에 애드온나오면 뒷부분 수정
			...
			resolve: async (src, args, ctx, info) => {
				try {
					...
					if (args.name && args.name.length > 50)
						return throwError(errors.etc('이름은 50자 이내로 입력해주세요.'), ctx);
			...
			resolve: async (src, args, ctx, info) => {
				try {
					...
					if (user.state !== 'ACTIVE')
						return throwError(errors.etc(userEmail + ' 유저는 활성화 상태가 아닙니다.'), ctx);
					...
					await ctx.prisma.purchaseLog.create({
						data: {
							type:
								plan.planLevel !== null
									? 'PLAN'
									: (plan.externalFeatureVariableId as NexusGenAllTypes['PurchaseLogType']),
							planInfo: JSON.stringify(etcPlanData),
							payAmount: plan.price,
							state: 'ACTIVE',
							payId: null,
					...
					const add = await ctx.prisma.purchaseLog.createMany({
						data: args.purchaseInputs.map((v: any) => {
							return {
								...
								planInfo:
									v.planInfoId === 2
										? `{"id":2,"planLevel":2,"name":"2단계","month":1,"price":99000,"externalFeatureVariableId":null}`
										: v.planInfoId === 3
											? `{"id":3,"planLevel":3,"name":"3단계","month":1,"price":132000,"externalFeatureVariableId":null}`
											: v.planInfoId === 4
												? `{"id":4,"planLevel":4,"name":"4단계","month":1,"price":149000,"externalFeatureVariableId":null}`
												: `{"id":2,"planLevel":2,"name":"2단계","month":1,"price":99000,"externalFeatureVariableId":null}`,
								payAmount:
									v.planInfoId === 2 ? 99000 : v.planInfoId === 3 ? 132000 : v.planInfoId === 4 ? 149000 : 99000,
								state: 'ACTIVE',
								payId: null,
								type: 'PLAN',
								userId: v.userId,
