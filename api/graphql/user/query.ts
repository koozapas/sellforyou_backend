...
export const query_user = extendType({
    ...
    definition(t) {
        ...
        t.field("selectUserLogsByUser", {
            ...
            resolve: async (src, args, ctx, info) => {
                try {
                    var result = await ctx.prisma.userLog.findMany({ where: { 
        ...
        t.crud.users({
            ...
            resolve : async (src,args,ctx,info,ori) => {
                try{
                    args.where = deepmerge<typeof args.where>(args.where, { id : {notIn : [224,367,487,517,549,550,578,579,587,588,597,602]}})
