...
export const mutation_user = extendType({
	...
	definition(t) {
		...
		t.field('findEmail', {
			...
			resolve: async (src, args, ctx, info) => {
				try {
					if (!regexPattern.phone.test(args.phone))
						return throwError(errors.etc('휴대폰 번호 형식이 잘못되었습니다.'), ctx);
					...
					if (!userInfo) return throwError(errors.etc('존재하지 않는 이용자입니다.'), ctx);
					const userList: any = userInfo
						.filter((v) => v.user.verificationNumber === args.verificationNumber)
						.map((v) => v.user.email);
			...
			resolve: async (src, args, ctx, info) => {
				try {
					...
					const user = await ctx.prisma.user.findFirst({
						where: { email: args.email, verificationNumber: args.verificationNumber },
					});
					...
					if (args.newPassword !== args.checkNewPassword)
						return throwError(errors.etc('신규 비밀번호가 일치하지 않습니다.'), ctx);
		...
		t.field('EditPasswordCreateVerification', {
			...
			resolve: async (src, args, ctx, info) => {
				try {
					...
					if (userData.userInfo.phone !== args.phoneNumber)
						return throwError(errors.etc('전화번호가 일치하지 않습니다.'), ctx);
					const verificationNumber = getRandomVerificationNumber(); //숫자를만들어냄
				...
				try {
						...
						switch (args.refCode) {
							...
							default: {
								...
								if (refCodeInfo.userInfo?.phone === tel) {
									return throwError(
										errors.etc('본인 계정은 추천인에 등록하실 수 없습니다. 새로고침 후 다시 시도해주세요.'),
										ctx,
									);
								}
					...
					const user = await ctx.prisma.user.create({
						data: {
							...
							verificationNumber: '',
							userInfo: {
								//다른 model에 data 삽입도 이런식으로 가능 하네 !
								create: {
									phone: tel,
									marginRate: 25,
		...
		t.field('signOutUserByEveryone', {
			//수성 임시 작성 refreshToken logout시 제거  로그아웃
			type: nonNull('String'),
			// args : {
		...
		t.field('signInUserByEveryone', {
			...
			resolve: async (src, args, ctx, info) => {
				try {
					...
					if (args.userType === 'EMAIL' && !compareSync(args.password, user.password))
						return throwError(errors.invalidUser, ctx);
			...
			resolve: async (src, args, ctx, info) => {
				try {
					...
					if (args.userType === 'KAKAO') {
						...
						if (existingUser > 0)
							return throwError(errors.etc('해당 카카오 계정은 다른 계정과 연동되어 있습니다.'), ctx);
					...
					if (args.userType === 'NAVER') {
						...
						if (existingUser > 0)
							return throwError(errors.etc('해당 네이버 계정은 다른 계정과 연동되어 있습니다.'), ctx);
			...
			resolve: async (src, args, ctx, info) => {
				try {
					...
					await ctx.prisma.userInfo.update({
						...
						data: {
							fixImageSubTop:
								args.fixImageSubTop && /^https?:/.test(args.fixImageSubTop) ? args.fixImageSubTop : undefined,
							...
							fixImageSubBottom:
								args.fixImageSubBottom && /^https?:/.test(args.fixImageSubBottom) ? args.fixImageSubBottom : undefined,
							fixImageBottom:
								args.fixImageBottom && /^https?:/.test(args.fixImageBottom) ? args.fixImageBottom : undefined, //즉 https로 시작하는지 여부를 보는거같다.
						},
					});
		...
		t.field('updateMyDataByUser', {
			...
			resolve: async (src, args, ctx, info) => {
				try {
					...
					if ((args.defaultShippingFee ?? 1) < 0)
						return throwError(errors.etc('기본 해외배송비는 음수일 수 없습니다.'), ctx);
		...
		t.field('changePasswordByUser', {
			...
			resolve: async (src, args, ctx, info) => {
				try {
					...
					if (!compareSync(args.currentPassword, user.password))
						return throwError(errors.etc('현재 비밀번호가 다릅니다.'), ctx);
			...
			resolve: async (src, args, ctx, info) => {
				try {
					...
					await ctx.prisma.userInfo.update({
						...
						data: { maxProductLimit: args.productLimit ?? null },
					});
					return true;
				} catch (e) {
					return throwError(e, ctx);
