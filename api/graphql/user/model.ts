...
export const getPurchaseInfo = async (
	prisma: PrismaClient,
	userId: number,
): Promise<NexusGenAllTypes['UserPurchaseInfo']> => {
	if (!userId) return { level: 0, levelExpiredAt: new Date(9990, 11, 31), history: '', additionalInfo: [] };

	const purchaseInfos = await prisma.purchaseLog.findMany({
		where: { userId: userId, state: 'ACTIVE', expiredAt: { gte: new Date() } },
	});
	...
	if (processedInfos2.length === 0)
		return {
			level: 0,
			levelExpiredAt: new Date(9990, 11, 31),
			history: JSON.stringify(purchaseInfos2),
			additionalInfo,
		};
	return {
		level: planLevel,
		levelExpiredAt: processedInfos2[0].expiredAt,
		history: JSON.stringify(purchaseInfos2),
		additionalInfo,
	};
};

export const getPurchaseInfo2 = async (
	prisma: PrismaClient,
	userId: number,
): Promise<NexusGenAllTypes['UserPurchaseInfo']> => {
	if (!userId) return { level: 0, levelExpiredAt: new Date(9990, 11, 31), history: '', additionalInfo: [] };
	const purchaseInfos = await prisma.purchaseLog.findMany({
		where: { userId, state: 'ACTIVE', expiredAt: { gte: new Date() } },
	});
