...
import {
  ...
  IPAJobCallbackRegistProdResultJson,
...
function isDoneResponse(
  response: IPAJobCallbackResponse<IPAJobCallbackRegistProdResultJson>
): response is IPAJobCallbackDoneResponse<IPAJobCallbackRegistProdResultJson> {
  ...
  try {
    ...
    if (isDoneResponse(response)) {
      ...
      try {
        ...
        if (userId) {
          const successfulMessage = results
            .filter((v) => v && v?.state === 1)
            .map((v) => `등록되었습니다. (SFY_${v!.productId.toString(36)})`)
            .join("\n");
          const failedMessage = results
            .filter((v) => v && v?.state !== 1)
            .map((v) => `등록되지 않았습니다. (SFY_${v!.productId.toString(36)}) 실패 사유: ${v!.reason}`)
            .join("\n");
